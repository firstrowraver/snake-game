...
export class FancyRenderer {
    ...
    // ---- Drawing helpers ----
    tileX(gx) { return this.playX + gx * TILE; }
    tileY(gy) { return this.playY + gy * TILE; }
    drawBrickWall() {
        const ctx = this.ctx;
    ...
    drawSnakeHead(gx, gy, dir) {
        ...
        ctx.lineWidth = 1.5;
        let tongueStartX, tongueStartY;
        switch (dir) {
            case Direction.Right:
                tongueStartX = x + TILE;
                tongueStartY = y + TILE / 2;
                ctx.beginPath();
    ...
    drawSnakeTail(gx, gy, prevGx, prevGy) {
        const ctx = this.ctx;
        const x = this.tileX(gx);
        const y = this.tileY(gy);
        const cx = x + TILE / 2;
